...
void spin(int cycles){
	...
	ros::Publisher RobotNode_stage_pub = n.advertise<geometry_msgs::Twist>("robot_12/cmd_vel",1000); 
...
void StageOdom_callback(nav_msgs::Odometry msg)
{
	...
	py = -14 + msg.pose.pose.position.y;
...
void visit(){
	ROS_INFO("The second relative(r12) Enters");
	navigate(0,1.5);
	navigate(1,11);
	navigate(2,8.5);
	navigate(1,1);
...
ros::init(argc, argv, "RobotNode12");
...
ros::Subscriber StageOdo_sub = n.subscribe<nav_msgs::Odometry>("robot_12/odom",1000, StageOdom_callback);
ros::Subscriber StageLaser_sub = n.subscribe<sensor_msgs::LaserScan>("robot_12/base_scan",1000,StageLaser_callback);


ros::Subscriber visit_sub = n.subscribe<project1::move>("robot_12/visit",1000,visit_callback);
