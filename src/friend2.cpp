...
void visit(){
	ROS_INFO("The second friend Enters");
	navigate(0,1.5);
	navigate(1,10);
	navigate(2,8.5);
	navigate(1,2);
	
	ROS_INFO("The second friend eats");
	spin(200);
	// Leave
	navigate(3,2);
	navigate(0,8.5);
...
ros::init(argc, argv, "RobotNode11");
...
ros::Subscriber StageOdo_sub = n.subscribe<nav_msgs::Odometry>("robot_11/odom",1000, StageOdom_callback);
ros::Subscriber StageLaser_sub = n.subscribe<sensor_msgs::LaserScan>("robot_11/base_scan",1000,StageLaser_callback);


ros::Subscriber visit_sub = n.subscribe<project1::move>("robot_11/visit",1000,visit_callback);
ros::Rate loop_rate(loopRate);
